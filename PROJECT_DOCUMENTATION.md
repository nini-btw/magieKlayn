...
- **A documented bug, found while testing, not fixed**: `app/api/products/[id]/route.test.ts` has a test explicitly titled `BUG: ...` proving `updateProductSchema` (built as `createProductSchema.partial()`) still applies the base schema's `.default(...)` values to fields a `PUT` caller omits — so a genuinely partial update (e.g. `{ isSoldOut: true }`) silently resets `isActive`/`isNew`/`gender`/`inspiredBy`/`notes` back to their defaults instead of leaving them untouched. No live caller triggers this today (`ProductForm`'s `handleSave` always submits the full form state), but it's a real trap for the next partial-update caller — including the `/api-docs` Swagger UI, whose own doc comment calls `PUT`'s body "Partial Product fields to update." Left unfixed deliberately (out of scope for a test-suite-only pass) but the test fails once `product.schema.ts` is corrected, at which point update the test per its own inline comment.
