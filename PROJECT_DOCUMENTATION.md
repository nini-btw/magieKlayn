...
|---|---|---|
| Next.js | `16.2.11` | Framework — App Router, Server Components, Server Actions, Route Handlers |
...
- **Supabase over a bespoke Postgres+auth stack**: gets managed Postgres, file storage, and an Auth service in one product, useful for a solo developer. Trade-off: the app doesn't actually use Supabase Auth's own user model as the source of truth — it layers a custom `admin_users` table on top and synthesizes Supabase sessions with a deterministic password (see [§9](#9-authentication--authorization)) — a workaround that suggests Supabase Auth wasn't a perfect fit for a single-admin-account use case, adding complexity rather than removing it.
...
- **Reason for choice**: the developer wanted the `admin_users` table (with `bcrypt` password hashing they control) to remain authoritative, while still getting Supabase's cookie-session machinery "for free" instead of hand-rolling one. The chosen approach: verify the password against `admin_users.passwordHash`, then create/update a **shadow Supabase Auth user** with a deterministic synthetic password (`` `auth_${email}_fixed_password_v1` `` — literally in the code, `src/infrastructure/auth/supabase-auth.ts:22`) purely so `signInWithPassword` can be called to mint a real Supabase session cookie.
- **Trade-offs accepted**: this is the single biggest security wart in the codebase. The synthetic password is a **known, predictable string derived only from the email address** — anyone who knows (or guesses) an admin's email and knows this source code (which they will, since it's likely in a public or shared repo) can compute the "real" Supabase password without ever needing the bcrypt-hashed one, then sign in directly against Supabase's own auth endpoint, bypassing `admin_users` entirely if `getAdminSession()`'s admin-table check were ever skipped anywhere. `getAdminSession()` does re-verify `admin_users` membership, so today this is contained, but it's a landmine for a future maintainer who doesn't understand why. The code's own comment flags it: `"Simple consistent password for Supabase Auth / In production, use a more secure approach."`
...
3. Compute a **deterministic synthetic password**: `` `auth_${email}_fixed_password_v1` `` — a hardcoded string template, not a secret, not randomly generated.
4. Using the Supabase **service-role admin client**, either update an existing Supabase Auth user's password to that synthetic value, or create one (`email_confirm: true`, `user_metadata: { role: "admin" }`) if none exists yet.
5. Sign in via the regular (cookie-writing) Supabase client with `signInWithPassword({ email, password: <synthetic> })` — this is what actually sets the session cookies the browser will send on subsequent requests.
...
**Authentication/session security**: see [§9](#9-authentication--authorization) in full. Headline concern: the deterministic synthetic Supabase password (`auth_${email}_fixed_password_v1`) is a genuine weakness, mitigated only by requiring the service-role key to provision it and by the `admin_users` re-check on every session read. No password complexity requirements are enforced anywhere in code for the `admin_users.passwordHash` — whatever was hashed at account-creation time (done manually, outside the app UI) is accepted as-is.
...
**Outdated/regret-worthy dependencies**: nothing found to be meaningfully outdated as of this writing — the stack (Next 16.2.11, React 19.2.4, Drizzle 0.45.2) is current/bleeding-edge rather than stale. The main "dependency" concern is architectural rather than version-related (see above).
