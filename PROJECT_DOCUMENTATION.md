...
- Root layout (`app/layout.tsx`) emits `Organization` JSON-LD (name/url/logo/`sameAs`); `app/shop/[slug]/page.tsx` emits `Product`/`Offer` JSON-LD (price in DZD, stock availability); `app/shop/layout.tsx` server-fetches the product list to emit `ItemList` JSON-LD, alongside `app/shop/page.tsx`'s own server-rendered product grid.
