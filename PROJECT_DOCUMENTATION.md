...
- **Reason for choice** (as implemented, likely unintentional): a `src/middleware/i18n.ts` file used to set up `createMiddleware(i18nConfig)` correctly as next-intl expects — but it lived at `src/middleware/i18n.ts`, not at the Next.js-recognized location. **This specific Next.js version (16.3.0) has renamed the `middleware.ts` file convention to `proxy.ts`** (per this repo's own `AGENTS.md` warning to consult `node_modules/next/dist/docs/` before assuming standard Next.js conventions apply). Because there was no `proxy.ts`/`middleware.ts` at the project root or `src/` root, that file was **never executed** — it was dead code, and has since been deleted (see [§20](#20-known-issues--technical-debt)). Locale resolution runs entirely through `app/layout.tsx`'s `getLocaleAndMessages()`, which reads a `NEXT_LOCALE` cookie server-side (falling back to `defaultLocale = "fr"` — target market is Algeria), and `<LanguageSwitcher>` sets that cookie and reloads the page.
- **Trade-offs accepted**: locale detection is cookie-only — a first-time visitor always gets `fr` (the default, chosen because the target market is Algeria) regardless of browser language, until they manually switch. There is also a **second, independent locale mechanism** in `app/template.tsx` that manages `<html dir>` via `localStorage`/a custom event, separate from the cookie the layout reads — the two are not obviously kept in sync (see [§20](#20-known-issues--technical-debt)).
...
├── i18n.config.ts                   locales ['en','fr','ar'], defaultLocale 'fr', tz 'Africa/Algiers'
...
- **`i18n.config.ts`** — `locales = ['en', 'fr', 'ar']`, `defaultLocale = 'fr'` (target market is Algeria), `timeZone: 'Africa/Algiers'`.
...
- Root layout (`app/layout.tsx`) emits `Organization` JSON-LD (name/`alternateName`/url/logo/`sameAs`); `app/shop/[slug]/page.tsx` emits `Product`/`Offer` JSON-LD (price in DZD, stock availability); `app/shop/layout.tsx` server-fetches the product list to emit `ItemList` JSON-LD, alongside `app/shop/page.tsx`'s own server-rendered product grid.
- **Brand-name misspelling coverage**: `app/layout.tsx`'s `BRAND_NAME_VARIANTS` array (~20 phonetic/spelling variants customers actually search with, e.g. "Magic Klein", "Maji Klayn", "Magiclaine", plus a couple of Arabic-transliteration variants) feeds both the `keywords` meta tag and the Organization JSON-LD's `alternateName` field — `alternateName` is the mechanism Google's entity resolution actually uses to match a typo'd query to this brand, `keywords` is a low-cost addition for other engines.
- `app/shop/page.tsx` is an **async Server Component** (fetches via `getAllProducts()`, deduped per-request with `app/shop/layout.tsx`'s fetch via the shared `cache()`-wrapped server action) that passes data to a client child, `app/shop/ShopPageClient.tsx`, which owns only the sort interactivity. It used to be a `"use client"` page fetching from `/api/products` in a `useEffect`, which left its server-rendered HTML an empty shell — Google Search Console flagged the resulting thin content as a soft 404 before this was fixed.
- `<head>` metadata (title/description/keywords/OG/Twitter) on the root layout and shop listing is **pinned to French** (`messages/fr.json`), independent of the visitor's cookie-selected UI locale — the target market is Algeria, and crawlers never send the `NEXT_LOCALE` cookie so they'd otherwise always see the `en` default. `<html lang>` and the rest of the UI still resolve per-visitor as before; only the metadata surface is fixed to French.
...
**A Jest test suite exists** (`jest.config.ts`, `jest.setup.ts`), added after this section documented its absence — 214 tests across 25 suites at time of writing. Configured via `next/jest` (per this Next.js version's own docs at `node_modules/next/dist/docs/01-app/02-guides/testing/jest.md`), split into two Jest `projects`:

- **`node` project** — `src/domain/**`, `src/application/**`, `src/infrastructure/**`, `app/api/**` (all matched by `*.test.ts`). Runs under Node, not jsdom, because `app/api/**/route.ts` handlers use the Web Fetch API (`Request`/`Response`/`Headers` from Node's own undici), not a DOM.
- **`jsdom` project** — `src/presentation/**` (`*.test.ts`/`*.test.tsx`). Runs under jsdom for React Testing Library component tests. Needed a `transformIgnorePatterns` override beyond what `next/jest` provides by default, since `next-intl`/`use-intl`/`@formatjs`/`intl-messageformat` ship pure ESM in `node_modules` and `next/jest` only lets custom config *append* to its default ignore pattern, not replace it — see the comment in `jest.config.ts` for the mechanics.
...
**What's covered** (prioritized by value, not exhaustive — see "what's NOT covered" below):
- **Pure business logic, in depth**: `src/domain/rules/cart.rules.ts` (box-count boundary math, coffret fee), `src/domain/entities/delivery.ts`/`order.ts` helpers, both `src/domain/validation/*.schema.ts` zod schemas, `src/application/services/cart.service.ts`, `src/infrastructure/rate-limit/limiter.ts` (including fake-timer window-reset behavior), `src/infrastructure/yalidine/config.ts`.
- **Redux**: `cart.slice.ts` and `ui.slice.ts` — every reducer/action, all memoized selectors, and the box-color-trimming invariant (`boxColors.length` never exceeds what the cart supports after a quantity/removal change).
- **API routes, as integration tests** (adapters/external services mocked at the module boundary via `jest.mock()` — see below): `POST /api/orders` is the priority, covering the happy path, the **security-critical price-tamper re-validation from the security audit** (client sends a low price, DB price is asserted to win), rate-limit exhaustion (429), zod rejection, the coffret box-count-exceeds-cart rejection, and the `store_pickup` fee-override anti-tamper path. Also `GET/POST /api/orders`, `GET/POST /api/products`, `GET/PUT/DELETE /api/products/[id]`, and `POST /api/upload` (including `sniffImageType()`, exported from the route specifically to unit-test each magic-byte signature — JPEG/PNG/GIF/WebP — plus a spoofed-Content-Type rejection test).
- **Components**: all of `src/presentation/components/ui/`'s primitives (`Button`, `Badge`, `Input`/`Textarea`, `QuantityStepper`, `Select`, `EmptyState`) via React Testing Library, plus `ProductCard`, `CartDrawer`, and `Header` as integration tests (real Redux store + `next-intl`, dispatched actions asserted against store state, using the existing `data-testid` selectors rather than inventing new ones).
- ~~A partial-update bug found while writing this suite~~ — **RESOLVED.** `updateProductSchema` used to be `createProductSchema.partial()`, and zod's `.partial()` only makes a field optional to *provide* — fields with `.default(...)` in the base schema (`gender`/`isActive`/`isNew`/`isSoldOut`/`inspiredBy`/`notes`) still applied that default when a `PUT` caller omitted them, silently resetting them instead of leaving them untouched. No live caller triggered this (`ProductForm`'s `handleSave` always submits the full form state), but it was a real trap for `/api-docs`'s own documented "Partial Product fields to update" contract. Fixed in `src/domain/validation/product.schema.ts` by building `updateProductSchema` from the same field validators *without* their defaults, rather than deriving it from the defaulted create schema — `app/api/products/[id]/route.test.ts` now asserts a partial `PUT` body leaves other fields untouched.

**What's explicitly NOT covered** (out of scope for this suite, not silently skipped):
- True Server Components/layouts (`app/layout.tsx`, `app/shop/layout.tsx`, `app/admin/(dashboard)/layout.tsx`, `app/shop/[slug]/page.tsx`) — Next's own Jest docs state async Server Components aren't Jest-testable yet ("we recommend using E2E tests for async components").
- `"use server"` action modules (`app/actions.ts`, `app/admin/actions.ts`) — need the Next server runtime or heavy mocking of `next/headers`/`next/cache`.
- `src/infrastructure/auth/supabase-auth.ts` (`adminLogin`/`getAdminSession` themselves, not just call sites) — heavily coupled to `@supabase/ssr`/`@supabase/supabase-js`/`next/headers`, all created at call time from `process.env`; a genuine unit test would need to mock all of them plus the DB, better suited to a focused follow-up than this general pass.
- `src/infrastructure/yalidine/stopdesk-resolver.ts`'s fuzzy commune-name matching, `src/infrastructure/telegram/telegram-notification.service.ts`, real Yalidine `fetch` retry/backoff logic — all fetch-mocking-heavy, lower priority than the checkout route itself (which already covers the security-relevant orchestration around them).
- `ProductForm.tsx`/`WilayaCommuneSelect.tsx` (heavier client-fetch components) and most `app/**/page.tsx` Client Components.
- Any E2E/Playwright/Cypress layer — the `data-testid` attributes below remain ready for one if built later.

**`data-testid` selectors reused, not reinvented**: `checkout-button`, `cart-button`, `cart-count`, `product-card`, `add-to-cart-button` are exercised by the component integration tests above. The remaining ones (`checkout-form`, `place-order-button`, `order-success`, `login-form`, `login-button`, `login-error`, `product-toggle`, `save-product-button`, `product-detail`) still await coverage — likely candidates for the next round (E2E or the currently-uncovered pages/forms above).

**Mocking strategy**: no dependency injection exists anywhere in `src/infrastructure/`/`src/application/` (every adapter is a module-level singleton built from `process.env` at import time — see §3/§9), so all route/service tests mock at the **module boundary** via `jest.mock("@/infrastructure/db/product.adapter")` etc., not constructor injection. `src/presentation/test-utils.tsx`'s `renderWithProviders()` is the shared helper for component tests — a real (but per-test-configurable) Redux store plus `NextIntlClientProvider` fed the real `messages/en.json` catalog.
...
**Full tech stack**: Next.js 16.3.0 (App Router, `reactCompiler: true`) · React 19.2.4 · TypeScript `^5` (strict) · Tailwind CSS v4 (CSS-first, no config file) · Drizzle ORM 0.45.2 + `postgres`(-js) driver → Supabase Postgres · Supabase Auth (`@supabase/ssr`) + custom `admin_users` table hybrid · Supabase Storage (product images) · Redux Toolkit + `react-redux` (cart/UI global state, no Zustand/Context/Jotai) · `next-intl` (en/fr/ar, RTL) · `react-hook-form` + `zod` (checkout/contact forms, plus server-side API validation) · Framer Motion (animations) · `next-swagger-doc` + `swagger-ui-react` (interactive API docs at `/api-docs`) · `@sentry/nextjs` (client/server/edge monitoring) · `bcryptjs` (admin password hashing) · Telegram Bot API (order alerts) · Yalidine API (delivery) · Jest + React Testing Library (unit/integration tests, see [§16](#16-testing)). **No** E2E framework, **no** ESLint config, **no** CI/CD, **no** Docker, **no** `.env.example`.
